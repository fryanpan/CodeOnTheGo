 ...
 *     `./gradlew :random-xkcd-plugin:assemblePluginDebug`) and copy
 *     `random-xkcd-plugin/build/plugin/random-xkcd-debug.cgp` into
 ...
 *     file exists.
 *
 * Recommended: option 1 for stability (asset lives with the test). The
 * androidTest asset path keeps the .cgp size bounded (xkcd is a few KB
 * per APK + brotli; Forms and Maps are bigger but still under a few MB).
 *
 * Either way, after the .cgp is in place call
 * `forcePluginManagerReload(...)` so the host picks it up without a full
 * relaunch.
 */
...
object PluginTestSetup {
     ...
     *
     * @param assetPath path under `androidTest/assets/`, e.g. `plugins/random-xkcd-debug.cgp`
